...
u32 ShowGameCheckerInfo(const char* path) {
    ...
    if (type & GAME_CIA) snprintf(contents_str, 64, STR_CONTENTS_IN_CIA_FOUND_TOTAL, content_found, content_count);
    else snprintf(contents_str, 64, STR_CONTENTS_IN_CIA_TOTAL, content_count);

    char conid_str[32] = { '\0' };
    if (type & (GAME_CIA|GAME_TIE)) snprintf(conid_str, 64, STR_CONSOLE_ID_N, console_id);
